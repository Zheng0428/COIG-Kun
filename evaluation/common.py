...
def should_process_file(card_id, file_index, total_cards=2):
    if file_index%32==card_id:
        return True
    else:
        return  False
    #return file_index%(card_id+1)==0
    # total_datasets = 31
    # avg_datasets_per_card = total_datasets // total_cards
    # extra_datasets = total_datasets % total_cards
    #
    # if card_id < extra_datasets:
    #     start_dataset = (avg_datasets_per_card + 1) * card_id
    #     end_dataset = start_dataset + avg_datasets_per_card
    # else:
    #     start_dataset = (avg_datasets_per_card + 1) * extra_datasets + avg_datasets_per_card * (card_id - extra_datasets)
    #     end_dataset = start_dataset + avg_datasets_per_card - 1
    #
    # return start_dataset <= file_index <= end_dataset
...
def load_questions(question_file: str, begin: Optional[int], end: Optional[int], tokenizer, model_id="baichuan-chat"):
    ...
    if "label" in model_id:
        for line in open(question_file):
            ...
            questions.append({"turns": [ques["output"]], "category": "QA", "question_id": question_id,
        ...
        for line in open(question_file):
            ...
            questions.append(
                {"turns": [f"指令: 【{ques['instruction']}】 回复："],
                 "category": "QA",
                 "question_id": ques["question_id"], "dataType": 0,
                 "instruction": ques["instruction"], "output": ques["output"]})
    elif "answer" in model_id:
        for line in tqdm.tqdm((open(question_file))):
            ques = json.loads(line)

            fixed_text = f"{ques['output']},Combining the above information. Please answer the question"
            available_tokens_for_instruction = 512 - len(tokenizer.tokenize(fixed_text))

            instruction_tokens = tokenizer.tokenize(ques['instruction'])
            if len(instruction_tokens) > available_tokens_for_instruction:
                instruction_tokens = instruction_tokens[:available_tokens_for_instruction]

            truncated_instruction = tokenizer.decode(tokenizer.convert_tokens_to_ids(instruction_tokens),
                                                     skip_special_tokens=True)

            question_text = f"{ques['output']},请回答:{truncated_instruction}"
...
def chat_compeletion_openai(model, conv, temperature, max_tokens):
    ...
    for _ in range(API_MAX_RETRY):
        try:
            ...
            openai.api_key = "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"
            # openai.api_base="https://api.chatkore.com"
