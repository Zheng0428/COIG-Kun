...
import sys
sys.path.append('/cpfs/29cd2992fe666f2a/user/huangwenhao/xw/Humpback-CH/baichuan2/evaluation')
...
from model.llama_condense_monkey_patch import (
    ...
    def raise_warning_for_old_weights(self, model):
        if isinstance(model, LlamaForCausalLM) and model.model.vocab_size > 32000:
            warnings.warn(
                ...
                "2. Use the old conversation template by `python3 -m serve.cli --model-path /path/to/vicuna-v0 --conv-template one_shot`\n"
...
class BaichuanAdapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        ...
        if "label" in model_path.lower():
            return get_conv_template("baichuan-label")
        elif "point" in model_path.lower():
            return get_conv_template("baichuan-point")
        elif "answer" in model_path.lower():
            return get_conv_template("baichuan-answer")
        else:
            return get_conv_template("baichuan-stem")
...
class InternLMChatAdapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        if "label" in model_path.lower():
            return get_conv_template("internlm-label")
        elif "point" in model_path.lower():
            return get_conv_template("internlm-point")
        else:
            return get_conv_template("internlm-answer")
...
class YiAdapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        if "label" in model_path.lower():
            return get_conv_template("Yi-label")
        elif "point" in model_path.lower():
            return get_conv_template("Yi-point")
        elif "answer" in model_path.lower():
            return get_conv_template("Yi-answer")
        else:
...
class Llama2Adapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        if "label" in model_path.lower():
            return get_conv_template("llama-2-label")
        elif "point" in model_path.lower():
            return get_conv_template("llama-2-point")
        elif "answer" in model_path.lower():
            return get_conv_template("llama-2-answer")
        else:
...
class QwenChatAdapter(BaseModelAdapter):
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        ...
        elif "stem" in model_path:
            return get_conv_template("qwen-stem")
        elif "point" in model_path:
            return get_conv_template("qwen-point")
